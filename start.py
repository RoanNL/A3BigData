...
def main():
    ...
    if not os.path.exists("venv"):
        executar_comando([sys.executable, "-m", "venv", "venv"], "Criando Ambiente Virtual (venv)")
    else:
    ...
    if os.path.exists(caminho_download):
        executar_comando([python_venv, caminho_download], "Verificando base de dados do DATASUS")
    
    # Executar a aplicação principal
    print("\n" + "="*45)
    print(" 🎯 Tudo pronto! Iniciando a aplicação (main.py)...")
    print("="*45 + "\n")
